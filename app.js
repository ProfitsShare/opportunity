...
if (pdfBtn) {
    ...
    if (data.isSubscribed === true) {
        ...
        pdfBtn.onclick = (e) => {
            ...
            // open your pdf
            window.open("https://github.com/ProfitsShare/opportunity/edit/main/reader.html", "_blank");
        };
...
function switchAuth(type) {
    ...
    if (type === "login") {
        ...
        referralStep.classList.remove("active");

    } else {
        referralStep.classList.add("active"); // 👈 start from step 1
...
setView("login");
    ...
    userRef.once("value").then(snapshot => {
        ...
        const balance = (userData?.balance || 0.00).toFixed(2);
...
function listenInviteCodePopup(uid) {
    ...
    userRef.on("value", (snapshot) => {
        ...
        const box = document.getElementById("inviteMessageBox");
        const btn = document.getElementById("getCodeBtnPopup");
...
btn.onclick = () => {
    ...
    if (!user) {
        alert("Login first to get your referral code");
setView("login");
        return;
    }
...
function getCode() {
    ...
    if (!user) {
        alert("Login first to get your code");
...
document.getElementById("referralStep").addEventListener("submit", async (e) => {
  ...
  const code = document.getElementById("step_referral").value.trim();
  ...
  try {
    const refSnapshot = await database.ref("referrals/" + code).once("value");
    ...
    // ✅ SAVE CODE
    document.getElementById("reg_referral").value = code;

    // ✅ SWITCH TO REGISTER FORM
    document.getElementById("referralStep").classList.remove("active");
